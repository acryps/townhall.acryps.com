...
export class MarketSeedMatcher extends MarketIterationGenerator {
	logger = new Logger('seed').task('match');

	async generate(innovations: Commodity[] = []) {
		const openSeedQuery = () => this.database.stockSeed
			.where(seed => seed.commodityId == null);

		const openSeedCount = await openSeedQuery().count();

		if (!openSeedCount) {
			return;
		}

		const openSeed = await openSeedQuery().skip(Math.floor(Math.random() * openSeedCount)).first();
		const trader = await TradingEntity.from(await openSeed.owner.fetch(), this.database);

		this.logger.log(`match ${openSeed.id.split('-')[0]} ${openSeed.sourceName} for ${trader.name}`);

		const commodity = await this.findProduct(openSeed, innovations);
	...
	findProduct(seed: StockSeed, innovations: Commodity[]) {
		return new Promise<Commodity>(async done => {
			const interpreter = new Interpreter('smart');

			const commodities = [
				...innovations,
				...(await this.getRandomCommodities(100))
			];

			let commodity: Commodity;

			interpreter.addTool('noMatch', [
				{ name: 'reason', type: String }
			], async reason => {
				seed.matchReason = reason;
			});

			interpreter.addTool('match', [
				{ name: 'id', type: String },
				{ name: 'reason', type: String }
			], async (id, reason) => {
				commodity = commodities.find(commodity => commodity.id.split('-')[0] == id.replace('id=', ''));
				...
				this.logger.log(`match ${seed.sourceName} to ${commodity.name}`);
			...
			await interpreter.execute(
				new SystemMessage(`
					Your job is to find a matching commodity given the product I provide.
					...
					We will add more commodities later on.

					The people have now written down their belogings, and we have to somehow merge the lists.
					A lot of things will not match, and that is okay, because we will do this matching again when the commodity list gets longer.
				`),

				new UserMessage(await this.compileCompactCommoditiesList('Available Commodities', commodities)),
