...
export class RelocateTenancyTileServer extends ShapeTileServer {
	constructor(
		app: ManagedServer,
		database: DbContext
	) {
		super(
			app,
			'relocate-tenancy/:dwellingId',

			async (parameters) => {
				const sourceDwelling = await database.dwelling.find(parameters.dwellingId);
				...
				// the property tenants are currently relocating from - highlighted, not pickable
				shapes.push({
					fill: '#e74c3c',
					stroke: '#000',
				...
				for (let property of properties) {
					...
					shapes.push({
						...
						name: `${emptyCount}/${dwellings.length}\n${ownerName}`
					});
				}
