...
export class MapComponent extends Component {
	...
	async onload() {
		this.location.x = +this.parameters.x;
		this.location.y = +this.parameters.y;
		this.location.zoom = +this.parameters.zoom;
	...
	render(child) {
		...
		return <ui-map>
			...
			<ui-controls>
				...
				<ui-control ui-active={!!this.tube}>
					XR

					<ui-control-extend>
						<ui-control ui-active={!this.tube} ui-click={() => {
							delete this.tube;
						...
						{this.tubes.map(tube => <ui-control ui-active={this.tube == tube} ui-click={() => {
							this.tube = tube;
				...
				<ui-control ui-active={!!this.findLayer(BoroughLayer)} ui-click={() => this.toggleLayer(new BoroughLayer(world, this))}>
					B
				</ui-control>

				<ui-control ui-active={!!this.findLayer(StreetLayer)} ui-click={() => this.toggleLayer(new StreetLayer(world, this))}>
					S
				</ui-control>

				<ui-control ui-active={!!this.findLayer(PropertyLayer)} ui-click={() => this.toggleLayer(new PropertyLayer(world, this))}>
					P
				</ui-control>

				<ui-control ui-active={!!this.findLayer(TrainLayer)} ui-click={() => this.toggleLayer(new TrainLayer(world, this))}>
					T
				</ui-control>

				<ui-control ui-active={!!this.findLayer(WaterLayer)} ui-click={() => this.toggleLayer(new WaterLayer(world, this))}>
					W
				</ui-control>


				<ui-control ui-active={!!this.draw} ui-click={() => {
					if (this.draw) {
	...
	updateHighlight() {
		if (this.child?.parameters.id) {
			for (let layer of this.mapInner.querySelectorAll("svg")) {
				if (layer.id == this.child?.parameters.id) {
	...
	setLocationUpdateTimer() {
		...
		this.locationParametersUpdateTimer = setTimeout(() => {
			if (this.route.component == Application.router.getActiveRoute()?.clientRoute.component) {
				this.parameters.x = this.location.x;
				this.parameters.y = this.location.y;
				this.parameters.zoom = this.location.zoom;
			}
		}, 1000);
	}
}
